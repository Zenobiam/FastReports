...
namespace FastReport.Forms
{
    ...
    /// </summary>
    public partial class WelcomeForm : ScaledSupportingForm
    {
        ...
        {
            InitializeComponent();
            this.banner.Image = ResourceLoader.GetBitmap("welcome.png");
        ...
        private void CheckRtl()
        {
            ...
            if (Config.RightToLeft)
            {
                ...
                int templeft = panelLeft.Left;
                panelLeft.Left = panelRight.Left;
                panelRight.Left = templeft;
                ...
                this.lblNew.Left = lblNew.Parent.Width - lblNew.Width;
            }
        }
                                    ...
                                    object tag)
        {
            ...
            if (trimText && !DpiHelper.HighDpiEnabled)
                trim(b);
        ...
        private bool trim(Control control)
        {
            ...
            bool trimmed = false;
            int i = txt.Length;
            int iconWidth = DpiHelper.ConvertUnits(30);
        ...
        protected override void Scale()
        {
            ...
            foreach(Control c in panelLeft.Controls)
            {
                if (!(c is Label))
                {
                    c.Font = ParseFontSize(c.Font, 9.5f);
                    c.Location = new Point(c.Location.X, prevBottom);
                    prevBottom = c.Bottom;
                }
                else
                {
                    c.Font = ParseFontSize(c.Font, 11.25f);
                    prevBottom = c.Bottom + DpiHelper.ConvertUnits(3);
                    (c as Label).AutoSize = false;
                }
                c.Width = c.Parent.Width - DpiHelper.ConvertUnits(getX());
                trim(c);
            }
            foreach (Control c in panelRight.Controls)
            {
                c.Font = DpiHelper.ConvertUnits(c.Font, true);
                if (!(c is Label))
                {
                    c.Font = ParseFontSize(c.Font, 9.5f);
                    c.Location = new Point(c.Location.X, prevBottom);
