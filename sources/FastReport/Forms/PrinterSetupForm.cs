  ...
  internal partial class PrinterSetupForm : BaseDialogForm
  {
    ...
    private bool Collapsed
    {
      ...
      set
      {
        ...
        if (value)
        {
          ...
          btnOk.Top = gbPageRange.Bottom + (int)(FormRatio * 15);
        ...
        else
        {
          ...
          btnOk.Top = gbOther.Bottom + (int)(FormRatio * 15);
          btnCancel.Top = btnOk.Top;
          btnMoreOptions.Top = btnOk.Top;
        }
        ClientSize = new Size(ClientSize.Width, btnOk.Bottom + (int)(FormRatio * 10));
      }
    }
    ...
    private void cbxPrinter_DrawItem(object sender, DrawItemEventArgs e)
    {
      if (e.Index >= 0)
      {
        ...
        g.DrawImage(GetImage(88), e.Bounds.X + 4, e.Bounds.Y);
        TextRenderer.DrawText(g, (string)cbxPrinter.Items[e.Index], e.Font, 
          new Point(e.Bounds.X + (int)(FormRatio * 30), e.Bounds.Y), e.ForeColor);
      }
    }
...
#if !MONO
      using (Bitmap bmp = GetBitmap(resName))
#else
    ...
    private void cbxPrintMode_DrawItem(object sender, DrawItemEventArgs e)
    {
      if (e.Index >= 0)
      {
...
#if !MONO
        using (Bitmap bmp = GetBitmap(resNames[e.Index]))
#else
        using (Bitmap bmp = ResourceLoader.GetBitmap(resNames[e.Index]))
#endif
        {
          g.DrawImage(bmp, e.Bounds.X + (int)(FormRatio * 3), e.Bounds.Y + (int)(FormRatio * 3));
        }
        ...
        g.DrawString((string)cbxPrintMode.Items[e.Index], cbxPrintMode.Font, SystemBrushes.WindowText, 
          e.Bounds.X + (int)(FormRatio * 70), e.Bounds.Y + (e.Bounds.Height - sz.Height) / 2);
      }
    }
    ...
    public PrinterSetupForm()
    {
        ...
        {
            base.Scale();
            cbxPrintMode.ItemHeight = DpiHelper.ConvertUnits(cbxPrintMode.ItemHeight);
        }
