...
namespace FastReport.Forms
{
    internal partial class WatermarkEditorForm : BaseDialogForm
    {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                dialog.Filter = Res.Get("FileFilters,AllImages");
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
#if DOTNET_4
                        if (dialog.FileName.IndexOf("svg") > -1)
                        {
                            SVGObject svg = new SVGObject();
                            svg.SetSVGByPath(dialog.FileName);
                            Watermark.Image = svg.GetImage();
                        }
                        else
#endif
