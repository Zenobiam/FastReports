...
namespace FastReport.Design.StandardDesigner
{
  ...
  public class DesignerMenu : Bar, IDesignerPlugin
  {
    ...
    private void miView_PopupOpen(object sender, PopupOpenEventArgs e)
    {
      ...
      foreach (IDesignerPlugin plugin in Designer.Plugins)
      {
        if (plugin is ToolWindowBase)
        {
          ...
          // ReinitDpiSize is called earlier than windows, so we resize the image
    ...
    public void UpdateUIStyle()
    {
      Style = UIStyleUtils.GetDotNetBarStyle(Designer.UIStyle);
        ...
        public void ReinitDpiSize()
        {
            ...
            miFileOpen.Image = Res.GetImage(1);
            miFileSave.Image = Res.GetImage(2);
            miFileSaveAll.Image = Res.GetImage(178);
            miFilePreview.Image = Res.GetImage(3);
            miFileSelectLanguage.RecalcSize();
            miEditUndo.Image = Res.GetImage(8);
            miEditRedo.Image = Res.GetImage(9);
            miEditCut.Image = Res.GetImage(5);
            miEditCopy.Image = Res.GetImage(6);
            miEditPaste.Image = Res.GetImage(7);
            miEditDelete.Image = Res.GetImage(51);
            miEditCopyPage.Image = Res.GetImage(6);
            miEditDeletePage.Image = Res.GetImage(12);
            miEditGroup.Image = Res.GetImage(17);
            miEditUngroup.Image = Res.GetImage(16);
            miEditFind.Image = Res.GetImage(181);
            miViewStartPage.Image = Res.GetImage(179);
            miDataAdd.Image = Res.GetImage(137);
            miDataShowData.Image = Res.GetImage(72);

            miWindowCloseAll.Image = Res.GetImage(202);

            miHelpContents.Image = Res.GetImage(90);
    ...
    public DesignerMenu(Designer designer) : base()
    {
      ...
      miEditCopyPage = CreateMenuItem(Res.GetImage(6), Designer.cmdCopyPage.Invoke);
