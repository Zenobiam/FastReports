...
namespace FastReport.Export.Pdf
{
    ...
    {
        private const float KAPPA1 = 1.5522847498f;
        private const float KAPPA2 = 2 - KAPPA1;
        ...
        private void DrawPDFUnderline(int fontNumber, Font font, float x, float y, float width, float offsX, Color color, bool transformNeeded, StringBuilder sb)
        {
            ...
            float factor = pDF_TTF_DIVIDER * font.Size * dpiFX * PDF_DIVIDER;
        ...
        private void DrawPDFFillRect(float Left, float Top, float Width, float Height, FillBase fill, StringBuilder sb)
        {
            ...
            else if (IsFillableGradientGrid(fill))
                DrawPDFVectorGradientFill(Left, Top, Width, Height, fill, sb);
        }
        ...
        private string DrawStyledLine(float left, float top, float right, float bottom, LineStyle lineStyle, float width)
        {
            ...
            sb.Append(FloatToString(width)).AppendLine(" w").AppendLine("2 J");
        ...
        private string DrawPDFDash(LineStyle lineStyle, float lineWidth)
        {
            ...
            else
            {
                ...
                return ExportUtils.StringFormat("[{0}] 0 d", result.ToString());
            }
        }
        ...
        {
            GraphicsPath clone = p.Clone() as GraphicsPath;
            FillPDFGraphicsPath(SizeF.Empty, clone, new SolidBrush(c), sb, curvesInterpolationText,
                new System.Drawing.Drawing2D.Matrix(PDF_DIVIDER, 0, 0, -PDF_DIVIDER, x, y));
        }
