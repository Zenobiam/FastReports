...
namespace FastReport.Export.BIFF8
{
    ...
    internal class OLE_Stream : BIFF8_Stream
    {
        ...
        internal void Read()
        {
            ...
            for (int i = 0; i < PropetiesCount; i++)
            {
                ...
                UInt32 cbSection = ReadUint();    // Section size in bytes
                UInt32 cProreties = ReadUint();   // Number of Properties in the Set
    ...
    internal class BIFF8_Font
    {
        ...
        */
        ...
        public BIFF8_Font(ExportIEMStyle style, ushort ColorIdx)
        {
            ...
            this.fontHeight = (ushort)(12 * Math.Round(f.Height * DrawUtils.ScreenDpiFX));
    ...
    internal class BIFF8_Hyperlink
    {
        // TODO: always has default value?
        internal ushort rwFirst = 0;
        internal ushort rwLast = 0;
        internal ushort colFirst = 0;
        internal ushort colLast = 0;

        internal uint optionFlags;
        string huperlink;
        bool is_url;

        private void Write_URL(StreamHelper stream)
        {
            byte[] GUID_Moniker = { 0xE0, 0xC9, 0xEA, 0x79, 0xF9, 0xBA, 0xCE, 0x11, 0x8C, 0x82, 0x00, 0xAA, 0x00, 0x4B, 0xA9, 0x0B };

            stream.WriteBytes(GUID_Moniker);
            uint calc_len = (uint)(huperlink.Length * 4);
            stream.WriteUint(calc_len);
            for (int i = 0; i < huperlink.Length; i++)
            {
                stream.WriteUshort((ushort)huperlink[i]);
            }
            stream.WriteUshort(0); // NULL-terminated
        }

        private void Write_HyperlinkToLocalFile(StreamHelper stream)
        {
            byte[] GUID_FileMoniker = { 0x03, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46 };

            stream.WriteBytes(GUID_FileMoniker);
            // There is no way to make portable implementation of this function
        }

        internal void Write(StreamHelper stream)
        {
            byte[] GUID_StdLink = { 0xd0, 0xC9, 0xEA, 0x79, 0xF9, 0xBA, 0xCE, 0x11, 0x8C, 0x82, 0x00, 0xAA, 0x00, 0x4B, 0xA9, 0x0B };

            optionFlags = 0x3;

            ushort record_size = (ushort)(32 + (16 + 4 + huperlink.Length * 2 + 2));

            stream.WriteUshort(0x01b8);         // Hyperlink
            stream.WriteUshort(record_size);    // Record size
            stream.WriteUshort(rwFirst);
            stream.WriteUshort(rwLast);
            stream.WriteUshort(colFirst);
            stream.WriteUshort(colLast);

            stream.WriteBytes(GUID_StdLink);
            stream.WriteUint(2); // Unknown value defined in BIFF8 spec
            stream.WriteUint(optionFlags);

            Write_URL(stream);
        }

        internal BIFF8_Hyperlink(string URL)
        {
            this.huperlink = URL;
            is_url = URL.Contains("://");
        }
    ...
    internal class BIFF8_Sheet : ArrayList
    {
        ...
        internal ArrayList url_list = new ArrayList();
        ...
        */
        ...
        internal void Write_URLs(StreamHelper stream)
        {
            foreach (BIFF8_Hyperlink hlink in url_list)
                hlink.Write(stream);
        }
    ...
    internal class Workbook : BIFF8_Stream
    {
        ...
        internal void Prepare(ExportMatrix export_matrix, bool page_breaks, string name_postfix)
        {
            ...
            string name = export_matrix.Report.PreparedPages.GetPage(0).Name + name_postfix;
            BIFF8_Sheet sheet = new BIFF8_Sheet(name);
            this.sheet_list.Add(sheet);

            for (ushort x = 0; x < export_matrix.Width - 1; x++)
            {
                ushort width = (ushort)(34 * (export_matrix.XPosById(x + 1) - export_matrix.XPosById(x)));
                sheet.colinfo_list.Add(new BIFF8_Colinfo(x, width, empty_cell_style));
            }
                    ...
                    if (i != -1)
                    {
                        ...
                        if (Obj.Counter < dy)
                        {
                            if (Obj.Counter == 0)
                            {
                                ...
                                else if (Obj.IsNumeric && Obj.Value != null)
                                {
                                ...
                                {
                                    BIFF8_Hyperlink link = new BIFF8_Hyperlink(Obj.URL);
                                    sheet.url_list.Add(link);
                                }
                            }
                            else
                            {
                            ...
                            if (!Obj.IsText) // Picture
                            {
                                if (Obj.Width > 0 && Obj.Counter == 0)
                                {
                                    ...
                                    sheet.pictures_list.Add(drawing);
                                }
                            }
                            ...
                            if ((Obj.Counter == dy) && (dx > 1 || dy > 1))
                            {
                                ...
                                sheet.cell_ranges.Add(single_range);
                            }
                        }
                    }
                    else
            ...
            this.Write_Palette(); // PALETTE
                                  // Skip many records at this point
            foreach (BIFF8_Sheet sheetItem in this.sheet_list)
            {
                sheetItem.Write_Globals(this);
            }
            ...
            foreach (BIFF8_Sheet sheetItem in this.sheet_list)
            {
                ...
                sheetItem.Write(this, page_breaks, export_matrix);
            }
    ...
    /// </summary>
    public partial class Excel2003Document : ExportBase
    {
        ExportMatrix matrix;
        int page_num;
        ...
        private void Prepare(string name_postfix)
        {
            ...
            workbook.Prepare(matrix, pageBreaks, name_postfix);
