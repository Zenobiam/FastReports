...
namespace FastReport.Export.OoXML
{
            ...
            if (relations.Count != 0)
            {
                ...
                foreach (OoXMLBase relation_item in relations)
                {
                    if (relation_item is OoHyperlink)
                    {
                        ...
                        "Target=" + Quoted(item.URL) +
                        "TargetMode=" + Quoted(relation_item.FileName) + "/>");
                    }
                    else
                    {
    ...
    class OoXMLCoreDocumentProperties : OoXMLBase
    {
        ...
        public void Export(OOExportBase OoXML)
        {
            ...
            if (Subject.Length != 0)
                ExportUtils.WriteLn(file, "<dc:subject>" + Subject.Replace("<", "&lt;").Replace(">", "&gt;") + "</dc:subject>");
    ...
    class OoXMLApplicationProperties : OoXMLBase
    {
        ...
        public void Export(OOExportBase OoXML)
        {
            ...
            ExportUtils.WriteLn(file, "<vt:lpstr>Page1</vt:lpstr>");
            ExportUtils.WriteLn(file, "</vt:vector>");
            ExportUtils.WriteLn(file, "</TitlesOfParts>");
