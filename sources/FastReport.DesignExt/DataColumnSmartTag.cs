...
namespace FastReport
{
  ...
  {
    private DataSourceBase rootDataSource;
        ...
        foreach (Relation relation in Obj.Report.Dictionary.Relations)
        {
          if (relation != rel && relation.Enabled && relation.ChildDataSource == data && relation.ParentDataSource != null)
          {
            ...
            if (DataColumn.IndexOf(GetDataColumn(item)) == 0)
              item.Font = new Font(item.Font.Name, DpiHelper.ParseFontSize(9), FontStyle.Bold);
            else
              item.Font = new Font(item.Font.Name, DpiHelper.ParseFontSize(9), FontStyle.Regular);
    ...
    {
      dataColumn = GetDataColumn(sender as ToolStripMenuItem);
      ItemClicked();
    }
    ...
    private string GetDataColumn(ToolStripMenuItem item)
    {
      ...
      result = rootDataSource.Alias + "." + result;
      return result;
    }
    ...
    protected override void CreateItems()
    {
      ...
      if (DataColumn == "")
        noneItem.Font = new Font(noneItem.Font.Name, DpiHelper.ParseFontSize(9), FontStyle.Bold);
      else
        noneItem.Font = new Font(noneItem.Font.Name, DpiHelper.ParseFontSize(9), noneItem.Font.Style);
      Menu.Items.Add(noneItem);

      // find the datasource to display its columns
      rootDataSource = FindRootDataSource();
      if (rootDataSource != null)
        AddDataSource(Menu.Items, rootDataSource);
    }
