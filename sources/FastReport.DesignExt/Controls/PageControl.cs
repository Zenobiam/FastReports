...
#if MONO
      int textHeight = (int)g.MeasureString(text, font, textWidth).Height;
      g.DrawString(text, font, SystemBrushes.ControlText, 
          new RectangleF(left + 12, top + (height - textHeight) / 2, textWidth, textHeight), StringFormat.GenericDefault);
#else
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
      ...
      if (SelectorWidth > 0)
      {
        g.DrawRectangle(SystemPens.ControlDark, 
          new Rectangle(Padding.Left - DpiHelper.ConvertUnits(1), 0, Width - Padding.Left, Height - DpiHelper.ConvertUnits(1)));
