...
namespace FastReport.Controls
{
    ...
    public class TreeViewMultiSelect : TreeView
    {
        ...
        protected override void OnNodeMouseClick(TreeNodeMouseClickEventArgs e)
        {
            ...
            if (selectedNodes.Count > 1 && !control && !shift)
            {
                unpaintSelectedNodes();
                selectedNodes.Clear();
        ...
        protected override void OnAfterSelect(TreeViewEventArgs e)
        {
            ...
            {
                if (selectedNodes.Count > 0 && isSameLevel(SelectedNode, e.Node))
                {
                    TreeNode SelectNode = NewSelection == true ? SelectedNodes[SelectedNodes.Count - 1] : SelectedNodes[0];
                    NewSelection = false;
                    List<TreeNode> nodes = getNodesOnSameLevel(SelectNode);

                    int from = Math.Min(SelectNode.Index, e.Node.Index);
                    int to = Math.Max(SelectNode.Index, e.Node.Index);
                    bool needReverse = SelectNode.Index > e.Node.Index;
