  ...
  internal class ComponentRefComboBox : ComboBox
  {
    ...
    protected override void OnDrawItem(DrawItemEventArgs e)
    {
      ...
      if (e.Index >= 0)
      {
        ...
        e.Graphics.DrawImage(img, e.Bounds.X + DpiHelper.ConvertUnits(2), e.Bounds.Y + DpiHelper.ConvertUnits(1));
        using (Brush b = new SolidBrush(e.ForeColor))
        {
          e.Graphics.DrawString(c == null ? Res.Get("Misc,None") : c.Name, e.Font, b,
            e.Bounds.X + DpiHelper.ConvertUnits(24), e.Bounds.Y + (e.Bounds.Height - DpiHelper.ConvertUnits(DrawUtils.DefaultItemHeight, true)) / 2);
        }
      }
    }
