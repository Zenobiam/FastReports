...
namespace FastReport.Controls
{
  internal class BandComboBox : ComboBox
  {
    protected override void OnDrawItem(DrawItemEventArgs e)
    {
      ...
      e.Graphics.DrawImage(img, e.Bounds.X + DpiHelper.ConvertUnits(2), e.Bounds.Y);
      using (Brush b = new SolidBrush(e.ForeColor))
      {
        ...
        e.Graphics.DrawString(text, e.Font, b,
          e.Bounds.X + DpiHelper.ConvertUnits(24), e.Bounds.Y + (e.Bounds.Height - ItemHeight) / 2);
      }
    }
    ...
    public BandComboBox()
    {
      ...
      ItemHeight = DrawUtils.DefaultItemHeight + DpiHelper.ConvertUnits(2);
    }
  }
}
