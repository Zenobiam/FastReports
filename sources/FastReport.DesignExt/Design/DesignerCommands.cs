  ...
  public class CopyPageCommand : DesignerCommand
  {
      ...
      protected override bool GetEnabled()
      {
          return !Restrictions.DontCopyPage && ActiveReport != null && ActiveReportTab.ActivePage != null &&
              ActiveReport.Pages.Count > 0 && ActiveReportTab.ActivePage is ReportPage &&
    ...
    private string GetHelpFile()
    {
      ...
      try
      {
        ...
        if (File.Exists(helpFile))
          return helpFile;
        helpFile = Config.ApplicationFolder + ".." + Path.DirectorySeparatorChar + 
          "FastReport.Net Documentation" + Path.DirectorySeparatorChar + "FRNetUserManual.chm";
        if (File.Exists(helpFile))
          return helpFile;
      }
      catch
